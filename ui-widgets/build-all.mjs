...
async function main() {
    ...
    const entries = fg
        .sync("src/*/index.{tsx,jsx,js}", { dot: false, onlyFiles: true })
        .sort(); // stable order
    if (!entries.length) {
    ...
    const h = crypto
        .createHash("sha256")
        .update(String(pkg.version || ""), "utf8")
        .digest("hex")
        .slice(0, 6);
    ...
    function wrapEntryPlugin(virtualId, entryFile, cssPaths) {
        return {
            ...
            load(id) {
                ...
                // IMPORTANT:
                // - Keep the namespace import to evaluate the real entry (side effects like createRoot).
                // - Do NOT add a second bare import; it can cause duplicate React instances.
                return `
${cssImports}
...
`;
    ...
    for (const file of entries) {
        ...
        const viteConfig = {
            ...
            resolve: {
                // Ensure only one React/React-DOM instance is ever resolved
                dedupe: ["react", "react-dom"],
                // Helps resolve symlinked workspace deps to a single copy
                preserveSymlinks: false,
            },
            optimizeDeps: {
                // Pre-bundle these to avoid duplicated transitive graphs
                include: ["react", "react-dom", "@duffel/components"],
            },
            esbuild: {
                jsx: "automatic",
